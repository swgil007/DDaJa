...
public enum TokenType {
    ...
    REFRESH("refresh_token", 24 * 14) {
        ...
        public String createToken(CommonJWT commonJWT, String secretKey) {
            Date now = new Date();
            SecretKey key = Keys.hmacShaKeyFor(secretKey.getBytes(StandardCharsets.UTF_8));
            return Jwts.builder().setSubject(commonJWT.getJwt()).setIssuedAt(now)
                    .setExpiration(DateUtil.addHours(now, getMaxHour())).signWith(key).compact();
        }
    };
    ...
    public void setCookieConfigure(Cookie cookie) {
